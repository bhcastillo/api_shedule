...
import { IOrder, IOrderSucces } from 'interfaces/order.interface';
import { IShedule, ISheduleCount } from 'interfaces/shedule.interface';

import { startConnection } from '../database';


export class Shedule {
  orderSucess: IOrderSucces[] = [];
  clearShedule = async (req: Request, res: Response) => {
    const conn: any = await startConnection();
    await conn.query('TRUNCATE shedule');
    await conn.query(`INSERT INTO shedule (id,day,hour,assigned) VALUES
    (1, 1, 1, 0),
    (2, 1, 2, 0),
    (3, 1, 3, 0),
    (4, 1, 4, 0),
...
`)
  ...
  getShedule = async (req: Request, res: Response) => {
    const conn: any = await startConnection();
  ...
  generateShedule = async (req: Request, res: Response) => {
    const conn = await startConnection();
    const getOrders: any = await conn.query('Select * FROM _order');
    const getMark: any = await conn.query('SELECT * FROM mark');
    const orders = [...getOrders[0]];
    const marks: IMark[] = [...getMark[0]];
    const getShedule: any = await conn.query(`
      ...
      WHERE (day < 6 AND hour >= 5 AND hour <= 20 OR 
            ...
            assigned = 0
      GROUP by day`);
    const shedule: ISheduleCount[] = [...getShedule[0]];
    let i = 0;
    this.orderSucess = [];
    orders.map((order: IOrder) => {
      const mark: any = marks.find((e) => e.id === order.id_mark);
  ...
  resolveOrder(order: IOrder, mark: IMark, shedule: ISheduleCount[]) {
    ...
    for (let dayCurrently = 1; dayCurrently <= 6; dayCurrently++) {
      ...
      if (shedule[dayCurrently].hours < mark.construction_time) continue;
      else {
        const limitQuantityDay = Math.round(shedule[dayCurrently].hours / mark.construction_time)
        if (!limitQuantityDay) continue;
